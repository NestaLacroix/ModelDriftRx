...
class DriftRxConfig:
    ...
    healing_trigger_severity: DriftSeverity = DriftSeverity.MODERATE
    ...
    def psi_bins(self) -> int:
        """Alias for psi_num_bins, used by the detector."""
        return self.psi_num_bins

    @property
    def psi_thresholds(self) -> tuple[float, float, float]:
        """Return (low, moderate, severe) thresholds as a tuple."""
        return (self.psi_none, self.psi_low, self.psi_moderate)

    @property
    def ks_significance(self) -> float:
        """Alias for ks_significance_level, used by the detector."""
        return self.ks_significance_level
